...
class DiscoveryService {
  ...
  async searchUsers(
    ...
    try {
      ...
      if (params.filters) {
        ...
        if (params.filters.startupStage)
          queryString.append(
            'filters[startupStage]',
            params.filters.startupStage,
          );
        if (params.filters.isRecent !== undefined)
          queryString.append(
            'filters[isRecent]',
            params.filters.isRecent.toString(),
          );
        if (params.filters.isVerified !== undefined)
          queryString.append(
            'filters[isVerified]',
            params.filters.isVerified.toString(),
          );
        if (params.filters.isPublic !== undefined)
          queryString.append(
            'filters[isPublic]',
            params.filters.isPublic.toString(),
          );
        ...
        if (params.filters.location) {
          if (params.filters.location.city)
            queryString.append(
              ...
              params.filters.location.city,
            );
          if (params.filters.location.state)
            queryString.append(
              'filters[location][state]',
              params.filters.location.state,
            );
          if (params.filters.location.country)
            queryString.append(
              'filters[location][country]',
              params.filters.location.country,
            );
          if (params.filters.location.radius)
            queryString.append(
              'filters[location][radius]',
              params.filters.location.radius.toString(),
            );
          if (params.filters.location.coordinates) {
            queryString.append(
              'filters[location][latitude]',
              params.filters.location.coordinates.latitude.toString(),
            );
            queryString.append(
              'filters[location][longitude]',
              params.filters.location.coordinates.longitude.toString(),
            );
          }
        }

        if (params.filters.skills) {
          params.filters.skills.forEach(skill =>
            queryString.append('filters[skills][]', skill),
          );
        }
      }

      const response = await fetch(
        `${this.baseUrl}/discovery/search?${queryString}`,
        {
          method: 'GET',
          headers: {
  ...
  async discoverByLocation(
    ...
    try {
      ...
      if (params.coordinates) {
        ...
        queryString.append(
          'longitude',
          params.coordinates.longitude.toString(),
        );
      }
      ...
      const response = await fetch(
        `${this.baseUrl}/discovery/location?${queryString}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  async getRecentJoins(
    params: RecentJoinsParams = {},
  ): Promise<RecentJoinsResponse> {
    try {
      const queryString = new URLSearchParams();
      ...
      const response = await fetch(
        `${this.baseUrl}/discovery/recent-joins?${queryString}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  async getSuggestedConnections(
    params: SuggestedConnectionsParams = {},
  ): Promise<SuggestedConnectionsResponse> {
    try {
      const queryString = new URLSearchParams();
      ...
      if (params.page) queryString.append('page', params.page.toString());
      if (params.minScore)
        queryString.append('minScore', params.minScore.toString());

      if (params.includedReasons) {
        params.includedReasons.forEach(reason =>
          queryString.append('includedReasons[]', reason),
        );
      }
      if (params.excludedReasons) {
        params.excludedReasons.forEach(reason =>
          queryString.append('excludedReasons[]', reason),
        );
      }

      const response = await fetch(
        `${this.baseUrl}/discovery/suggestions?${queryString}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  async getSearchHistory(
    ...
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/discovery/search-history?${queryString}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  ): Promise<{ success: boolean; message: string; search: SavedSearch }> {
    try {
  ...
  async deleteSavedSearch(
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/discovery/saved-searches/${searchId}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
    ...
    for (const candidate of candidateUsers) {
      ...
      if (userInteractions) {
        const hasViewed = userInteractions.some(
          interaction =>
            interaction.targetUserId === candidate.userId &&
            interaction.action === 'view_profile',
        );
        if (hasViewed) {
  ...
  diversifySuggestions(
    ...
    for (const suggestion of suggestions) {
      ...
      if (
        suggestion.company &&
        seenCompanies.has(suggestion.company.toLowerCase())
      ) {
        if (seenCompanies.size < 3) {
          // Allow some duplicates if we have few companies
          shouldAdd = true;
        } else {
          shouldAdd = false;
        }
      ...
      if (
        suggestion.industry &&
        seenIndustries.has(suggestion.industry.toLowerCase())
      ) {
        if (seenIndustries.size < 5) {
          // Allow some duplicates if we have few industries
          shouldAdd = true;
        } else {
          shouldAdd = false;
        }
      ...
      if (shouldAdd) {
        ...
        if (suggestion.company)
          seenCompanies.add(suggestion.company.toLowerCase());
        if (suggestion.industry)
          seenIndustries.add(suggestion.industry.toLowerCase());
